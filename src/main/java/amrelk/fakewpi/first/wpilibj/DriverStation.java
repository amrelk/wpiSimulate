...
import amrelk.fakewpi.first.wpilibj.hal.AllianceStationID;
import amrelk.fakewpi.first.wpilibj.hal.ControlWord;
import amrelk.fakewpi.first.wpilibj.hal.HAL;
import amrelk.fakewpi.first.wpilibj.hal.MatchInfoData;
import amrelk.fakewpi.first.wpilibj.hal.PowerJNI;
  ...
  private static void reportErrorImpl(boolean isError, int code, String error,
    ...
    if (printTrace) {
      ...
      for (int i = stackTraceFirst; i < stackTrace.length; i++) {
        ...
        if (!haveLoc && !loc.startsWith("amrelk.fakewpi.first")) {
          locString = loc;
          haveLoc = true;
        }
      }
